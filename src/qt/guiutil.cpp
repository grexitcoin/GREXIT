...
{
    // Convert GREXIT:// to GREXIT:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("GREXIT://"))
    {
        uri.replace(0, 12, "GREXIT:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "GREXIT.lnk";
}
...
{
    return GetAutostartDir() / "GREXIT.desktop";
}

bool GetStartOnSystemStartup()
{
    boost::filesystem::ifstream GREXITFile(GetAutostartFilePath());
    if (!GREXITFile.good())
        return false;
    // Scan through file for "Hidden=true":
    std::string line;
    while (!GREXITFile.eof())
    {
        getline(GREXITFile, line);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        boost::filesystem::ofstream GREXITFile(GetAutostartFilePath(), std::ios_base::out|std::ios_base::trunc);
        if (!GREXITFile.good())
            return false;
        // Write a bitcoin.desktop file to the autostart directory:
        GREXITFile << "[Desktop Entry]\n";
        GREXITFile << "Type=Application\n";
        GREXITFile << "Name=GREXIT\n";
        GREXITFile << "Exec=" << pszExePath << " -min\n";
        GREXITFile << "Terminal=false\n";
        GREXITFile << "Hidden=false\n";
        GREXITFile.close();
    }
    return true;
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("GREXIT-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  GREXIT-qt [" + tr("command-line options") + "]                     " + "\n";
