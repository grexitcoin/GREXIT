...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #GREXITcoinTEST\r");
            Send(hSocket, "WHO #GREXITcoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #GREXITcoin\n");
            // randomly join #GREXITcoin00-#GREXITcoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #GREXITcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #GREXITcoin%02d\r", channel_number).c_str());
        };
